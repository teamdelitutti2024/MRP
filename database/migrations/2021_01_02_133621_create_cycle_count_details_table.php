<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class CreateCycleCountDetailsTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('cycle_count_details', function (Blueprint $table) {
            $table->increments('id');
            $table->decimal('system_quantity', 10, 2);
            $table->decimal('human_quantity', 10, 2);
            $table->boolean('adjustment_status');
            $table->text('adjustment_justification')->nullable();
            $table->dateTime('adjustment_date')->nullable();
            $table->integer('cycle_count_id')->unsigned();
            $table->integer('supply_id')->unsigned();
            $table->integer('measurement_unit_id')->unsigned();
            $table->integer('supply_reception_id')->unsigned();
            $table->integer('responsible_id')->unsigned()->nullable();

            $table->foreign('cycle_count_id')->references('id')->on('cycle_counts');
            $table->foreign('supply_id')->references('id')->on('supplies');
            $table->foreign('measurement_unit_id')->references('id')->on('measurement_units');
            $table->foreign('supply_reception_id')->references('id')->on('supply_receptions');
            $table->foreign('responsible_id')->references('id')->on('users');
        });

        Schema::enableForeignKeyConstraints();
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::disableForeignKeyConstraints();
        Schema::dropIfExists('cycle_count_details');
    }
}
