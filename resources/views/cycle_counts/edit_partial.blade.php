...
<div class="page-title">
    <h5><i class="fa fa-tag"></i> {{ __('Editar conteo parcial') }}</h5>
    <div class="btn-group">
...
<form action="{{ url('/cycle_counts/update_partial/' . $cycle_count->id) }}" role="form" method="POST" autocomplete="off">
    @csrf
                        ...
                        @foreach($cycle_count_details as $element)
                            <tr>
                                <td class="mandatory-field">
                                    <span>*</span>
                                    <p style="display: inline;" id="key-{{ $counter }}">{{ $element->sup->supply_key }}</p>
                                </td>
                                <td>
                                ...
                                <td>
                                    <select name="details[supply_location_id][]" class="form-control">
                                        @foreach($supply_locations as $supply_location)
                                            <option value="{{ $supply_location->id }}" {{ $supply_location->id == $element->supply_location_id ? 'selected' : '' }}>{{ $supply_location->location }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td><input type="text" class="form-control quantities" name="details[quantity][]" id="quantity-{{ $counter }}" value="{{ $element->counted_quantity }}" pattern="(0*[0-9][0-9]{0,6}(\.[0-9]{1,3})?|0+\.[0-9]{1,3})" title="{{ __('Número decimal con 7 digitos enteros y 3 cifras decimales como máximo') }}" required></td>
