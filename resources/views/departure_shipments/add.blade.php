...
<div class="page-title">
    <h5><i class="fa fa-tag"></i> {{ __('Agregar embarque de salida') }}</h5>
    <div class="btn-group">
        <a href="{{ url('/orders/detail/' . $order->id) }}" class="btn btn-link btn-lg btn-icon"><i class="fa fa-arrow-circle-left"></i>{{ __('Regresar') }}</a>
    </div>
</div>
<!-- /page title -->
...
<form action="{{ url('/departure_shipments/store') }}" role="form" method="POST" autocomplete="off" id="departure-shipment">
    @csrf
            ...
            <div class="form-group">
                <div class="row">
                    <div class="col-sm-12">
                        ...
                        <p>{{ __('ID Orden') }}: <a href="{{ url('/orders/detail/' . $order->id) }}">{{ $order->id }}</a></p>
                        <p>{{ __('Sucursal') }}: <a href="{{ url('/branches/detail/' . $order->branch_id) }}">{{ $order->branch->name }}</a></p>
                        <br>
                        <label><span class="mandatory">*</span> {{ __('Fecha de envío') }}</label>
                        <input type="text" class="form-control" name="shipment_date" id="shipment_date" value="{{ $today }}" readonly required>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            ...
            <span class="pull-right label label-info">{{ __('Total de embarque de salida') }}: $<span id="dinamic-total">0.00</span></span>
        </div>
        <div class="panel-body">
                ...
                <table class="table table-hover" id="details-table">
                    ...
                    <tbody>
                        ...
                        @foreach($order->order_products as $element)
                            <tr>
                                <td class="mandatory-field"><span>*</span>{{ $element->product->name }}</td>
                                ...
                                <input type="hidden" name="details[requested_quantity][]" value="{{ $element->quantity }}">
                                <td>{{ $element->product_size->name }}</td>
                                <td>$<p style="display: inline;" id="price-{{ $counter }}">{{ number_format($element->price, 2, '.', ',') }}</p></td>
                                <td><p style="display: inline;">{{ $element->quantity }}</p></td>
                                <td><input type="text" maxlength="3" class="form-control quantities" name="details[quantity][]" id="quantity-{{ $counter }}" value="" pattern="(100|[0-9][0-9]?)" title="{{ __('Número entero entre 0 y 100') }}" required></td>
