...
<div class="page-title">
    <h5><i class="fa fa-tag"></i> {{ __('Editar tamaño de producto') }}</h5>
    <div class="btn-group">
        <a href="{{ url('/products/detail/' . $product_size->product_id) }}" class="btn btn-link btn-lg btn-icon"><i class="fa fa-arrow-circle-left"></i>{{ __('Regresar') }}</a>
    </div>
</div>
<!-- /page title -->
...
<form action="{{ url('/product_sizes/update/' . $product_size->id) }}" role="form" method="POST" autocomplete="off" id="form-edit">
    @csrf
    <div class="panel panel-default">
        <div class="panel-heading">
            <h6 class="panel-title">{{ __('Información de tamaño de producto') }}</h6>
            <span class="pull-right label label-info">{{ __('Precio calculado') }}: $<span id="calculated-price">{{ $general_total }}</span></span>
        </div>
        <div class="panel-body">
            <div class="form-group">
                <div class="row">
                    <div class="col-sm-6">
                        ...
                        <input type="text" class="form-control" name="name" value="{{ $product_size->name }}" maxlength="60" required>
                    </div>
                    <div class="col-sm-3">
                        <label><span class="mandatory">*</span> {{ __('Clave') }}</label>
                        <input type="text" class="form-control" name="product_size_key" value="{{ $product_size->product_size_key }}" readonly>
                    </div>
                    <div class="col-sm-3">
                        <label><span class="mandatory">*</span> {{ __('Complejidad') }}</label>
                        <select name="complexity" class="form-control">
                            <option value="0" id="modal-complexity-0">{{ __('Ninguna') }}</option>
                            @foreach(config('dictionaries.product_complexities') as $key => $value)
                                <option value="{{ $key }}" {{ $key == $product_size->complexity ? 'selected' : '' }}>{{ __($value) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="form-group">
                <div class="row">
                    <div class="col-sm-3">
                        <label><span class="mandatory">*</span> {{ __('Multiplo de referencia para pedido') }}</label>
                        <input type="text" class="form-control" name="products_minimum_amount" value="{{ $product_size->products_minimum_amount }}" pattern="[1-9][0-9]?" title="{{ __('Número entero positivo con 2 cifras como máximo') }}" required>
                    </div>
                    <div class="col-sm-3">
                        <label><span class="mandatory">*</span> {{ __('Precio de venta') }}</label>
                        <input type="text" class="form-control" name="sale_price" value="{{ $product_size->sale_price }}" pattern="(0*[1-9][0-9]*(\.[0-9]{1,2})?|0+\.[0-9]{1,2})" title="{{ __('Número decimal mayor a 0 con 2 cifras decimales como máximo') }}"  required>
                    </div>
                </div>
            </div>
        </div>
                ...
                <table class="table table-hover" id="ingredients-table">
                    ...
                    <tbody>
                        @php 
                            ...
                            $counter = 0;
                        @endphp
                        @foreach($product_size->ingredients as $element)
                            @php
        ...
        <div class="panel-body">
            <div class="table-responsive">
                <table class="table table-hover" id="bake-table">
                    ...
                    <tbody>
                        @php $counter_bake = 0; @endphp
                        @foreach($bake_breads_data as $element)
                            <tr>
                                <td class="mandatory-field">
                                    <span>*</span>
                                    <p style="display: inline;" id="bkey-{{ $counter_bake }}">{{ $element->bake_bread->bake_bread_size_key }}</p>
                                </td>
                                <td>
                                    <select name="bake_breads[bake_bread_size_id][]" class="form-control bake_breads" id="b-{{ $counter_bake }}">
                                        @foreach($bake_breads as $bake_bread)
                                            <option value="{{ $bake_bread->id }}" {{ $element->bake_bread_size_id == $bake_bread->id ? 'selected' : '' }}>{{ $bake_bread->name }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td>
                                    <p style="display: none;" id="bcost-{{ $counter_bake }}">{{ $bake_breads_costs[$counter_bake] }}</p>
                                    <p style="display: none;" id="btotal-{{ $counter_bake }}">{{ $bake_breads_costs[$counter_bake] * $element->quantity }}</p>
                                    <input type="text" name="bake_breads[quantity][]" class="form-control bake_breads_quantities" id="bquantity-{{ $counter_bake }}" value="{{ $element->quantity }}" pattern="(0*[1-9][0-9]{0,6}(\.[0-9]{1,3})?|0+\.[0-9]{1,3})" title="{{ __('Número decimal mayor a 0 con 7 digitos enteros y 3 cifras decimales como máximo') }}" required>
                                </td>
                                <td>
                                    <div class="table-controls">
                                        <a href="#" class="btn btn-danger btn-icon btn-xs del-row-bake" id="delete_bake-{{ $counter_bake }}"><i class="fa fa-trash"></i></a>
                                    </div>
                                </td>
                            </tr>
                            @php $counter_bake++; @endphp
                        @endforeach
                    </tbody>
                </table>
            </div>
        ...
        <div class="panel-body">
            <div class="table-responsive">
                <table class="table table-hover" id="prepared-table">
                    ...
                    <tbody>
                        @php $counter_prepared = 0; @endphp
                        @foreach($prepared_products_data as $element)
                            <tr>
                                <td class="mandatory-field">
                                    <span>*</span>
                                    <p style="display: inline;" id="pkey-{{ $counter_prepared }}">{{ $element->prepared_product->product_key }}</p>
                                </td>
                                <td>
                                    <select name="prepared_products[prepared_product_id][]" class="form-control prepared_products" id="p-{{ $counter_prepared }}">
                                        @foreach($prepared_products as $prepared_product)
                                            <option value="{{ $prepared_product->id }}" {{ $element->prepared_product_id == $prepared_product->id ? 'selected' : '' }}>{{ $prepared_product->name }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td>
                                    <p style="display: none;" id="pcost-{{ $counter_prepared }}">{{ $prepared_products_costs[$counter_prepared] }}</p>
                                    <p style="display: none;" id="ptotal-{{ $counter_prepared }}">{{ $prepared_products_costs[$counter_prepared] * $element->quantity }}</p>
                                    <input type="text" name="prepared_products[quantity][]" class="form-control prepared_products_quantities" id="pquantity-{{ $counter_prepared }}" value="{{ $element->quantity }}" pattern="(0*[1-9][0-9]{0,6}(\.[0-9]{1,3})?|0+\.[0-9]{1,3})" title="{{ __('Número decimal mayor a 0 con 7 digitos enteros y 3 cifras decimales como máximo') }}" required>
                                </td>
                                <td>
                                    <div class="table-controls">
                                        <a href="#" class="btn btn-danger btn-icon btn-xs del-row-prepared" id="delete_prepared-{{ $counter_prepared }}"><i class="fa fa-trash"></i></a>
                                    </div>
                                </td>
                            </tr>
                            @php $counter_prepared++; @endphp
                        @endforeach
                    </tbody>
                </table>
            </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover" id="resources-table">
                    ...
                    <tbody>
                        ...
                        @foreach($resources_data as $element)
                            <tr>
                                <td class="mandatory-field">
                                    <span>*</span>
                                ...
                                <td>
                                    <p style="display: none;" id="rcost-{{ $counter_resources }}">{{ $element->resource->cost }}</p>
                                    <p style="display: none;" id="rtotal-{{ $counter_resources }}">{{ $element->resource->cost * ($element->production_time / $element->quantity_to_produce) }}</p>
                                    <input type="text" name="resources[production_time][]" class="form-control resources_times" id="rtime-{{ $counter_resources }}" value="{{ $element->production_time }}" pattern="(0*[1-9][0-9]{0,6}(\.[0-9]{1,2})?|0+\.[0-9]{1,2})" title="{{ __('Número decimal mayor a 0 con 7 digitos enteros y 2 cifras decimales como máximo') }}" required>
                                </td>
                                <td><input type="text" name="resources[quantity_to_produce][]" class="form-control resources_quantities" id="rquantity-{{ $counter_resources }}" value="{{ $element->quantity_to_produce }}" maxlength="3" pattern="(100|[1-9][0-9]?)" title="{{ __('Número entero entre 1 y 100') }}" required></td>
                                <td>
                                    <div class="table-controls">
                                        <a href="#" class="btn btn-danger btn-icon btn-xs del-row-resource" id="delete_resource-{{ $counter_resources }}"><i class="fa fa-trash"></i></a>
                                    </div>
                                </td>
                            </tr>
...
<script type="text/javascript">
    var counter = {{ $counter }};
    var total = {{ $total }};
    var counter_bake = {{ $counter_bake }};
    var counter_prepared = {{ $counter_prepared }};
    var counter_resources = {{ $counter_resources }};
    var general_total = {{ $general_total }}
