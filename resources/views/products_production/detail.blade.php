            ...
            <tbody>
                @foreach($production->production_details as $element)
                    <tr>
                        ...
                        <td><a href="{{ url('/products/detail/' . $element->product_id) }}">{{ $element->product_name }}</a></td>
                        <td><a href="{{ url('/product_sizes/edit/' . $element->product_size_id) }}">{{ $element->product_size_name }}</a></td>
...
<div class="panel panel-default">
    <div class="panel-heading">
        ...
        <a href="{{ url('/products_production/download_projection/' . $production->id) }}" class="pull-right btn btn-xs btn-success"><i class="fa fa-download"></i> {{ __('Descargar reporte de producción') }}</a>
    </div>
    <div class="table-responsive">
        <table class="table table-hover">
            <thead>
...
<div class="panel panel-default">
    <div class="panel-heading">
        <h6 class="panel-title">{{ __('Bases utilizadas') }}</h6>
    </div>
    <div class="table-responsive">
        <table class="table table-hover">
            <thead>
