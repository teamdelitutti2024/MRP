...
<div class="page-title">
    <h5><i class="fa fa-tag"></i> {{ __('Editar tamaño de base') }}</h5>
    <div class="btn-group">
        <a href="{{ url('/bake_bread_sizes') }}" class="btn btn-link btn-lg btn-icon"><i class="fa fa-arrow-circle-left"></i>{{ __('Regresar') }}</a>
    </div>
</div>
<!-- /page title -->
...
<form action="{{ url('/bake_bread_sizes/update/' . $bake_bread_size->id) }}" role="form" method="POST" autocomplete="off">
    @csrf
    <div class="panel panel-default">
        <div class="panel-heading">
            <h6 class="panel-title">{{ __('Información de tamaño de base') }}</h6>
        </div>
            ...
            <div class="form-group">
                <div class="row">
                    <div class="col-sm-6">
                        ...
                        <input type="text" class="form-control" name="name" value="{{ $bake_bread_size->name }}" maxlength="60" required>
                    </div>
                    <div class="col-sm-3">
                        <label><span class="mandatory">*</span> {{ __('Clave') }}</label>
                        <input type="text" class="form-control" name="bake_bread_size_key" value="{{ $bake_bread_size->bake_bread_size_key }}" readonly>
                    </div>
                    <div class="col-sm-3">
                        ...
                        <select name="complexity" class="form-control">
                            ...
                            @foreach(config('dictionaries.product_complexities') as $key => $value)
                                <option value="{{ $key }}" {{ $key == $bake_bread_size->complexity ? 'selected' : '' }}>{{ __($value) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <table class="table table-hover" id="ingredients-table">
                    ...
                    <tbody>
                        @php
                            ...
                            $counter = 0;
                        @endphp
                        @foreach($bake_bread_size->ingredients as $element)
                            @php
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <div class="table-responsive">
                <table class="table table-hover" id="bake-bread-sizes-table">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 25%">
                                <div class="table-controls">
                                    <a href="#" class="btn btn-default btn-icon btn-xs" id="add-bake-bread-size-row"><i class="fa fa-plus"></i></a>
                                </div>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        @php $counter_bake_bread_sizes = 0; @endphp
                        @foreach($bake_breads_data as $element)
                            <tr>
                                <td class="mandatory-field">
                                    <span>*</span>
                                    <p style="display: inline;" id="bkey-{{ $counter_bake_bread_sizes }}">{{ $element->related_bake_bread_size->bake_bread_size_key }}</p>
                                </td>
                                <td>
                                    <select name="bake_bread_sizes[related_bake_bread_size_id][]" class="form-control bake_bread_sizes" id="b-{{ $counter_bake_bread_sizes }}">
                                        @foreach($bake_bread_sizes as $bake_bread_size)
                                            <option value="{{ $bake_bread_size->id }}" {{ $element->related_bake_bread_size_id == $bake_bread_size->id ? 'selected' : '' }}>{{ $bake_bread_size->name }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td><input type="text" name="bake_bread_sizes[quantity][]" class="form-control" value="{{ $element->quantity }}" pattern="(0*[1-9][0-9]{0,6}(\.[0-9]{1,3})?|0+\.[0-9]{1,3})" title="{{ __('Número decimal mayor a 0 con 7 digitos enteros y 3 cifras decimales como máximo') }}" required></td>
                                <td>
                                    <div class="table-controls">
                                        <a href="#" class="btn btn-danger btn-icon btn-xs del-row" id="delete_bake_bread_size-{{ $counter_bake_bread_sizes }}"><i class="fa fa-trash"></i></a>
                                    </div>
                                </td>
                            </tr>
                            @php $counter_bake_bread_sizes++; @endphp
                        @endforeach
                    </tbody>
                </table>
            </div>
                    ...
                    <tbody>
                        ...
                        @foreach($prepared_products_data as $element)
                            <tr>
                                ...
                                <td><input type="text" name="prepared_products[quantity][]" class="form-control" value="{{ $element->quantity }}" pattern="(0*[1-9][0-9]{0,6}(\.[0-9]{1,3})?|0+\.[0-9]{1,3})" title="{{ __('Número decimal mayor a 0 con 7 digitos enteros y 3 cifras decimales como máximo') }}" required></td>
                                <td>
                                    <div class="table-controls">
                                        <a href="#" class="btn btn-danger btn-icon btn-xs del-row" id="delete_prepared-{{ $counter_prepared }}"><i class="fa fa-trash"></i></a>
                                    </div>
                                </td>
                            </tr>
...
<script type="text/javascript">
    var counter = {{ $counter }};
    var total = {{ $total }};
    var counter_bake_bread_sizes = {{ $counter_bake_bread_sizes }};
    ...
    $(document).ready(function() {
        ...
        $('#add-bake-bread-size-row').click(function(e) {
            e.preventDefault();
            ...
            html += '   <td class="mandatory-field">';
            html += '       <span>*</span>';
            html += '       <p style="display: inline;" id="bkey-' + counter_bake_bread_sizes + '"></p>'
            html += '   </td>'
            html += '   <td>';
            html += '       <select name="bake_bread_sizes[related_bake_bread_size_id][]" class="form-control bake_bread_sizes" id="b-' + counter_bake_bread_sizes + '">';
            ...
            html += '   </td>';
            html += '   <td>';
            html += '       <input type="text" name="bake_bread_sizes[quantity][]" class="form-control" pattern="(0*[1-9][0-9]{0,6}(\.[0-9]{1,3})?|0+\.[0-9]{1,3})" title="@lang('Número decimal mayor a 0 con 7 digitos enteros y 3 cifras decimales como máximo')" required>';
            html += '   </td>';
            html += '   <td>';
            html += '       <div class="table-controls">';
            html += '           <a href="#" class="btn btn-danger btn-icon btn-xs del-row" id="delete_bake_bread_size-' + counter_bake_bread_sizes + '"><i class="fa fa-trash"></i></a>';
            html += '       </div>';
            html += '   </td>';
            html += '</tr>';

            // First bake bread size
            var bake_bread_size_id = {{ $bake_bread_sizes[0]['id'] }};

            // Get bake bread size
            $.ajax({
                url: '{{ url('/bake_bread_sizes/get_row_v2') }}',
                data: 'id=' + bake_bread_size_id,
                dataType: 'json',
                type: 'post',
                cache: false,
                success: function(data) {
                    var internal_count = counter_bake_bread_sizes - 1;

                    $('#bkey-' + internal_count).text(data.bake_bread_size_key);
                }
            });

            counter_bake_bread_sizes++;
            
            $('#bake-bread-sizes-table > tbody').append(html);
        });

        $(document).on('change', '.bake_bread_sizes', function(e) {
        ...
        $('#add-prepared-row').click(function(e) {
            ...
            html += '   </td>';
            html += '   <td>';
            html += '       <input type="text" name="prepared_products[quantity][]" class="form-control" pattern="(0*[1-9][0-9]{0,6}(\.[0-9]{1,3})?|0+\.[0-9]{1,3})" title="@lang('Número decimal mayor a 0 con 7 digitos enteros y 3 cifras decimales como máximo')" required>';
            html += '   </td>';
            html += '   <td>';
            html += '       <div class="table-controls">';
            html += '           <a href="#" class="btn btn-danger btn-icon btn-xs del-row" id="delete_prepared-' + counter_prepared + '"><i class="fa fa-trash"></i></a>';
            html += '       </div>';
            html += '   </td>';
            html += '</tr>';
